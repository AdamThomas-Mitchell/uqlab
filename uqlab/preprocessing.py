...
def train_cal_test_split(X, y, train_size=0.4, cal_size=0.4, test_size=0.2, random_state=None):
    """
...
def scale_targets(y_train, y_cal, y_test):
    ...
    return y_train_sc, y_cal_sc, y_test_sc


def prepare_water_dimer_atom(atom_dataset, n_train, n_cal, n_test, random_state=None):
    """
    ...
    n_total_pts = n_train + n_cal + n_test
    atom_subset = atom_dataset.sample(n=n_total_pts, random_state=random_state)
    ...
    train_prop = n_train / n_total_pts
    cal_prop = n_cal / n_total_pts
    ...
    X_train, X_cal, X_test = scale_features(X_train, X_cal, X_test)
    y_train, y_cal, y_test = scale_targets(y_train, y_cal, y_test)
...
def prepare_water_dimer_system_data(n_train, n_cal, n_test, random_state=None):
    """
    ...
    :return:
    """
    waterDimer_dict = get_water_dimer_data()

    processed_waterDimer_dict = {
        key: prepare_water_dimer_atom(
            value,
            n_train=n_train,
            n_cal=n_cal,
            n_test=n_test,
...
def prepare_glycine_atom(atom_tuple, train_size, random_state=None):
    """
...
def prepare_glycine_system_data(train_prop, random_state=None):
    """
    Return dictionary of preprocessed data for each atom in water dimer system
    Returned dict is of form;
    ...
    :return:
    """
    glycine_dict = get_glycine_data()

    processed_glycine_dict = {
        key: prepare_glycine_atom(
            value,
            train_size=train_prop,
            random_state=random_state
        )
